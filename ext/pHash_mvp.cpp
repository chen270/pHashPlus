...
MVPRetCode ph_save_mvptree(MVPFile *m, DP **points, int nbpoints, int saveall_flag, int level, FileIndex *pOffset)
{
    ...
    else if (nbpoints <= LeafCapacity + 2)
    { /* leaf */
        uint8_t ntype = 0;
        MVPFile m2;
        m2.branchfactor = m->branchfactor;
    ...
    { /* internal node */
        ...
        { /* append new page to mainfile, unmap/map to it */
    ...
    if (ntype == 0)
    {
        ...
        if (sv1)
        {
            ...
            if (sv2)
            {
                ...
                { /* convert to internal node and add leaf nodes */
