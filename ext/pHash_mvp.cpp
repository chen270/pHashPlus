...
const char mvptag[] = "pHashMVPfile2023";

static int ph_selectvantagepoints(MVPFile *m, DP **points, int N, int &sv1_pos, int &sv2_pos) {
    if (N > 2) {
        sv1_pos = 0;
        sv2_pos = 1;
        float maxdist = 0.0f;
...
MVPRetCode _ph_map_mvpfile(uint8_t filenumber, off_t offset, MVPFile *m, MVPFile *m2, int use_existing) {
    ...
    if (filenumber == m->filenumber) {
        /* in the file denoted by m , must advance to page containing offset */
        m2->file_pos = offset;
        ...
        m2->buf = (char *)MapViewOfFile(fmhandle, FILE_MAP_WRITE, offset_hi, offset_lo, alloc_offset + m->pgsize);
        if (m2->buf == NULL) {
        ...
        /* open and map to new file denoted by m->filename and filenumber */
        char extfile[256];
        snprintf(extfile, sizeof(extfile), "%s%d.mvp", m->filename, filenumber);
        m2->fh = CreateFile(extfile, GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL,
                            OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
        if (m2->fh == INVALID_HANDLE_VALUE) {
            return PH_ERRFILEOPEN;
        }
        HANDLE fmhandle = CreateFileMapping(m2->fh, NULL, PAGE_READWRITE, 0, 0, fm_objname);
        if (fmhandle == NULL) {
            return PH_ERRMMAPCREATE;
        }
        m2->pathlength = m->pathlength;
        ...
        DWORD alloc_offset = alloc_offset_mask & offset;
        DWORD offset_hi = 0, offset_lo = alloc_unit;
        m2->buf = (char *)MapViewOfFile(fmhandle, FILE_MAP_ALL_ACCESS, offset_hi, offset_lo, alloc_offset + m->pgsize);
        if (m2->buf == NULL) {
            return PH_ERRMMAPVIEW;
        }
        m2->buf += alloc_offset;
...
MVPRetCode ph_query_mvptree(MVPFile *m, DP *query, int knearest, float radius, float threshold, DP **results,
                            int *count, int level) {
        ...
        for (pivot1 = 0; pivot1 < LengthM1; pivot1++) {
            if (d1 - radius <= M1[pivot1]) {
                ...
                if (d2 + radius >= M2[LengthM1 - 1 + pivot1 * LengthM1]) {
                     ...
                     * offset */
                    curr_pos =
                        start_pos + (BranchFactor - 1 + pivot1 * BranchFactor) * (sizeof(uint8_t) + sizeof(off_t));
                    m->file_pos = curr_pos;
                    memcpy(&filenumber, &m->buf[m->file_pos & offset_mask], sizeof(uint8_t));
                    m->file_pos++;
                    memcpy(&child_pos, &m->buf[m->file_pos & offset_mask], sizeof(off_t));
...
MVPRetCode ph_query_mvptree(MVPFile *m, DP *query, int knearest, float radius, float threshold, DP **results,
    ...
    m->fh = CreateFile(mainfile, GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_EXISTING,
    ...
    m->buf = (char *)MapViewOfFile(fmhandle, FILE_MAP_ALL_ACCESS, 0, 0, int_pgsize);
    if (m->buf == NULL) {
        return PH_ERRMMAPVIEW;
    }
    m->branchfactor = bf;
...
MVPRetCode ph_save_mvptree(MVPFile *m, DP **points, int nbpoints, int saveall_flag, int level, FileIndex *pOffset) {
    ...
    if (nbpoints <= 0) {
        ...
        last_pos = curr_pos + LeafCapacity * (2 * sizeof(float) + sizeof(off_t));
        off_t dp_pos;
    ...
    leafcleanup:
        ...
        if ((level > 0) && (saveall_flag == 1)) { /* append new page to mainfile, unmap/map to it */
            ...
            DWORD offset_hi = 0, offset_lo = alloc_unit;
            m->buf =
                (char *)MapViewOfFile(fmhandle, FILE_MAP_ALL_ACCESS, offset_hi, offset_lo, alloc_offset + m->pgsize);
            if (m->buf == NULL) {
                return PH_ERRMMAPVIEW;
            }
            m->buf += alloc_offset;
...
MVPRetCode ph_save_mvptree(MVPFile *m, DP **points, int nbpoints) {
    ...
    char mainfile[MAX_PATH];
    snprintf(mainfile, sizeof(mainfile), "%s.mvp", m->filename);
    m->fh = CreateFile(mainfile, GENERIC_READ | GENERIC_WRITE, FILE_SHARE_WRITE | FILE_SHARE_READ | FILE_SHARE_DELETE,
    ...
    if (ntype == 0) {
        ...
        if (sv1) {
            ...
            if (sv2) {
                ...
                } else if (Np < m->leafcapacity) { /* add new dp to existing link */
        ...
        for (pivot1 = 0; pivot1 < LengthM1; pivot1++) {
            if (d1 <= M1[pivot1]) {
                ...
                for (pivot2 = 0; pivot2 < LengthM1; pivot2++) {
                    if (d2 <= M2[pivot2 + pivot1 * LengthM1]) {
                        ...
                        if ((filenumber == 0) && (child_pos == 0)) {
                            ...
                            retcode = ph_save_mvptree(m, &new_dp, 1, 0, level + 2, &child_offset);
                ...
                if (d2 > M2[LengthM1 - 1 + pivot1 * LengthM1]) {
                    curr_pos = start_pos +
                               (m->branchfactor - 1 + pivot1 * m->branchfactor) * (sizeof(uint8_t) + sizeof(off_t));
                    m->file_pos = curr_pos;
                    memcpy(&filenumber, &m->buf[m->file_pos & offset_mask], sizeof(uint8_t));
                    m->file_pos++;
                    memcpy(&child_pos, &m->buf[m->file_pos & offset_mask], sizeof(off_t));
        ...
        if (d1 > M1[LengthM1 - 1]) {
            ...
            if (d2 > M2[LengthM1 - 1 + LengthM1 * LengthM1]) {
                curr_pos =
                    start_pos + (m->branchfactor - 1 + LengthM1 * m->branchfactor) * (sizeof(uint8_t) + sizeof(off_t));
                m->file_pos = curr_pos;
                memcpy(&filenumber, &m->buf[m->file_pos & offset_mask], sizeof(uint8_t));
                m->file_pos++;
                memcpy(&child_pos, &m->buf[m->file_pos & offset_mask], sizeof(off_t));
